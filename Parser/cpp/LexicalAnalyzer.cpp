...
LexicalAnalyzer* LexicalAnalyzer::_singleton = nullptr;
LexicalAnalyzer* LexicalAnalyzer::Singleton()
{
	if (_singleton == nullptr)
		_singleton = new LexicalAnalyzer();
...
LexicalAnalyzer::LexicalAnalyzer()
{
	...
	_keyword["FUNCTION"] = make_shared<Function>(0, 0);
...
TokenStream LexicalAnalyzer::Scan(istream& input)
{
	...
	while (input.good() && input.peek() != EOF)
	{
		...
		auto newtoken = (this->*_scanChar[input.peek()])(input);

		// Spaces are harmless as well as useless
		// Ignore them
		if ( !dynamic_pointer_cast<Space>(newtoken))
			result.emplace_back(newtoken);
	}
...
shared_ptr<Token> LexicalAnalyzer::ScanCharConst(istream& input)
{
	...
	return make_shared<Letter>(_line, _col, charcode);
}
